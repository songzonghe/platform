				...
				<div class="widget-header">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="utilDoctorTitle_index.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								...
								<fieldset>
									...
									<input type="text" id="query_title_name" placeholder="职称名称" value="${map.title_name}">
									<button type="submit" onclick="queryUtilDoctorTitle(0)" class="btn btn-sm btn-success">
										搜索
		...
		<table id="sample-table-1" class="table  table-bordered dataTable"> 
			<thead>
				<tr>
					...
					<th field="title_name"   onclick="querySort(this,'queryUtilDoctorTitle')"  >
						职称名称
					</th>
					<th field="title_sort"  type="num"   onclick="querySort(this,'queryUtilDoctorTitle')"  >
						排序
					</th>
					<th field="gxsj" width="170" onclick="querySort(this,'queryUtilDoctorTitle')">
						修改时间
					</th>
					<th field="bz"  onclick="querySort(this,'queryUtilDoctorTitle')">
						备注
					</th>
					<th width="180">
						操作
			...
			<tbody id="tbodyData">
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr id="tr${info.title_id}">
						...
						<td style="vertical-align: middle;">
							<button class="btn btn-xs btn-info" onclick="openEditPage('${info.title_id}')" title="编辑">
								编辑
							</button>
							<button class="btn btn-xs btn-danger" onclick="confirmDeleteDate('${info.title_id}','${info.title_id}')" title="删除">
								删除
							</button>
						</td>
					</tr>
...
<c:set var="cx" value="queryUtilDoctorTitle" />
...
<script src="${root}/form/util/doctor/title/js/utildoctortitle_main.js?<%=Math.random()%>"></script>
</body>
</html>