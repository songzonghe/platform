...
/************************系统生成方法开始******************************/
function queryUtilBatchNumber(page){
	if(isUndefinedAndEmpty(page)){
		page=0;
	}
	loadingDiv();
	var query_log_name = $("#query_log_name").val();
	var json = {
...
function openAddPage(){
	var jsonAjax = {
		"url" : "utilBatchNumber_addPage.do",
		"jsonData" : null,
		"title" : "批号添加",
		"icon" : "plus",
...
function getUpdateMainJson(id) {
	...
	var update_log_name = $("#update_log_name").val();
	var update_log_sort = $("#update_log_sort").val();
	var jsonObj = {
		"hospital_id" : update_hospital_id,
		"log_name" : update_log_name,
		"log_sort" : update_log_sort,
		"log_id" : id
	};
...
function addData(jsonObj){
	...
	var jsonAjax = {
		"url" : "utilBatchNumber_updateData.do",
		"jsonData" : jsonObj,
...
function openEditPage(id){
	...
	var jsonAjax = {
		"url" : "utilBatchNumber_editPage.do",
...
function confirmDeleteDate(id,name){
	...
	adminJS.confirm(jsonObj,function(e,id){
		...
		var jsonAjax = {
			'url' : 'utilBatchNumber_deleteData.do',
