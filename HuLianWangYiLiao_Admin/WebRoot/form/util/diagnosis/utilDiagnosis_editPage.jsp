	...
	<form class="form-horizontal" isValidate="y" onsubmit="return false;">
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<select class="select" data-find="find" id="update_icd_id" data-validate="required:分类名称不能为空哦！">
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<select class="select" data-find="find" id="update_diagnosis_name" data-validate="required:诊段名称不能为空哦！">
						<option value="">--请选择--</option>
						<c:forEach var="info" items="${listIcdDoctor.icdList}">
							<option value="${info.icd_name}" <c:if test="${info.icd_name==map.diagnosis_name}">selected</c:if> >${info.icd_name}</option>
						</c:forEach>
					</select>
				</div>
			</div>
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						...
						<option value="">--请选择--</option>
						<option <c:if test="${map.diagnosis_sex=='1'}">selected="selected"</c:if> value="1">男</option>
						<option <c:if test="${map.diagnosis_sex=='2'}">selected="selected"</c:if> value="2">女</option>
						<option <c:if test="${map.diagnosis_sex=='3'}">selected="selected"</c:if> value="3">不限制</option>
					</select>
					</span>
				</div>
			</div>
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						<input type="text" placeholder="最低年龄" id="update_diagnosis_dage" class="col-xs-12" data-validate="required:最低年龄不能为空哦！,number:只能是数字哦！" value="${map.diagnosis_dage}" >
					</span>
				</div>
			</div>
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						<input type="text" placeholder="最高年龄" id="update_diagnosis_tage" class="col-xs-12" data-validate="required:最高年龄不能为空哦！,number:只能是数字哦！" value="${map.diagnosis_tage}" >
					</span>
				</div>
			</div>
		</div>
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						...
						<option value="">--请选择--</option>
						<option <c:if test="${map.is_restriction=='0'}">selected="selected"</c:if> value="0">否</option>
						<option <c:if test="${map.is_restriction=='1'}">selected="selected"</c:if> value="1">是</option>
					</select>
					</span>
				</div>
			</div>
		...
		<div class="form-actions center">
			...
			<button type="submit" class="btn btn-sm btn-success" onclick="editDataValidate('${map.diagnosis_id}')">
				提交
