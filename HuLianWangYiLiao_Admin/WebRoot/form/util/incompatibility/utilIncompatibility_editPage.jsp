...
<div class="widget-main no-padding" style="width: 550px">
	<form class="form-horizontal" isValidate="y" onsubmit="return false;">
		...
		<%-- <div class="form-group field">
			...
			<div class="col-sm-5">
				<span class="block input-icon input-icon-right">
					<input type="text" placeholder="药品主键" id="update_drug_id" class="col-xs-12" data-validate="required:不能为空哦！" value="${map.drug_id}" >
				</span>
			</div>
		</div> --%>
				...
				<span class="block input-icon input-icon-right">
					<select  id="update_drug_id" class="col-xs-12 select" data-validate="" onchange="drugList()">
						...
						<c:forEach items="${drugList}" var="info">
							<option value="${info.drug_id}"<c:if test="${info.drug_id==map.drug_id}">selected</c:if> >${info.drug_name}</option>
						</c:forEach>
					</select>
				</span>
			</div>
			...
			<div class="col-sm-5 field">
				<span class="block input-icon input-icon-right">
					<select  id="update_drug_id2" class="col-xs-12 select" data-validate="" onchange="drugList2()">
						<option value="">请选择</option>
						<c:forEach items="${drugList}" var="info">
							<option value="${info.drug_id}"<c:if test="${info.drug_id==map.drug_id2}">selected</c:if> >${info.drug_name}</option>
						</c:forEach>
					</select>
				</span>
			</div>
		...
		<div class="form-actions center">
			...
			<button type="submit" class="btn btn-sm btn-success" onclick="editDataValidate('${map.incompatibility_id}')">
				提交
