	...
	styleUI : {
		jQueryUI : {
			...
			base : {
				...
				icon_first : "btn pagination Home",
				icon_prev : "btn pagination Prev",
				icon_next: "btn pagination Next",
				icon_end: "btn pagination End",
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td id='zDir' dir=\""+dir+"\">";
				str +="<span>每页显示 </span><select style=\"height:30px; display: inline-block;\" "+getstyle(stylemodule, 'pgSelectBox', false, 'ui-pg-selbox')+" role=\"listbox\" title=\""+($.jgrid.getRegional(ts,"defaults.pgrecs",ts.p.pgrecs) || "")+ "\">";
				var strnm;
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += str;
				pgl += "<td id='"+po[1]+"' class='"+btc+"'  title='"+ pot[1] +"'" +"><span " + getstyle(stylemodule, 'icon_prev', false, iconbase)+">上一页</span></td>";
				pgl += pginp !== "" ?pginp:"";
				pgl += "<td id='"+po[2]+"' class='"+btc+"' title='"+ pot[2] +"'" +"><span " + getstyle(stylemodule, 'icon_next',false, iconbase)+">下一页</span></td>";
			} else if (pginp !== "") { 
				pgl += pginp; 
			}
			if(dir==="ltr") { 
			...
			if(ts.p.viewrecords===true) {
				$("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<div dir='"+dir+"' style='text-align:left' class='ui-paging-info'></div>");
			}
...
$.jgrid.extend({
	...
	filterInput : function(my_ruleGroup,my_groupOp) {
		var p = $.extend(true, {
			defaultSearch : 'cn',
			groupOp : my_groupOp,
			searchAll : false,
		...
		return this.each(function(){
			...
			ruleGroup = '{"groupOp":"'+my_groupOp+'","rules":['+my_ruleGroup+']}';
