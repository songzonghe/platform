				...
				<div class="widget-header">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="utilCertificates_index.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								...
								<fieldset>
									...
									<input type="text" id="query_certificates_number" placeholder="编号" value="${map.certificates_number}">
									<button type="submit" onclick="queryUtilCertificates(0)" class="btn btn-sm btn-success">
										搜索
		...
		<table id="sample-table-1" class="table  table-bordered dataTable"> 
			<thead>
				<tr>
					...
					<th field="certificates_number"   onclick="querySort(this,'queryUtilCertificates')"  >
						编号
					</th>
					<th field="certificates_name"   onclick="querySort(this,'queryUtilCertificates')"  >
						证件名称
					</th>
					<th field="certificates_sort" type="num"  onclick="querySort(this,'queryUtilCertificates')"  >
						排序
					</th>
					<th field="gxsj" width="170" onclick="querySort(this,'queryUtilCertificates')">
						修改时间
					</th>
					<th field="bz"  onclick="querySort(this,'queryUtilCertificates')">
						备注
					</th>
					<th width="180">
						操作
			...
			<tbody id="tbodyData">
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr id="tr${info.certificates_id}">
						<td index="tdIndex" style="vertical-align: middle;">
							${i.index+1}
						</td>
						<td style="vertical-align: middle;">
							${info.certificates_number}
						</td>
						<td style="vertical-align: middle;">
							${info.certificates_name}
						</td>
						<td style="vertical-align: middle;">
							${info.certificates_sort}
						</td>
						<td style="vertical-align: middle;">
							${info.gxsj}
						</td>
						...
						<td style="vertical-align: middle;">
							<button class="btn btn-xs btn-info" onclick="openEditPage('${info.certificates_id}')" title="编辑">
								编辑
							</button>
							<button class="btn btn-xs btn-danger" onclick="confirmDeleteDate('${info.certificates_id}','${info.certificates_id}')" title="删除">
								删除
							</button>
						</td>
					</tr>
...
<c:set var="cx" value="queryUtilCertificates" />
...
<script src="${root}/form/util/certificates/js/utilcertificates_main.js?<%=Math.random()%>"></script>
</body>
</html>