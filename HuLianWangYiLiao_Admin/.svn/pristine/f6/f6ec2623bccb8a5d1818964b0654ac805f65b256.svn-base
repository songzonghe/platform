				...
				<div class="widget-box">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="reportForm_prescription_index.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								<input style="display: none;" id="mapStr" name="mapStr">
		 						<fieldset>
									...
									<option value="">请选择</option>
										<option <c:if test="${map.find_zt=='-1'}">selected="selected"</c:if> value="-1">未开方</option>
										...
										<option <c:if test="${map.find_zt=='6'}">selected="selected"</c:if> value="6">撤销处方</option>
									</select>
									<button type="submit" onclick="queryPrescriptionUser(0)" class="btn btn-sm btn-success">
										搜索
...
<c:set var="cx" value="queryPrescriptionUser" />
