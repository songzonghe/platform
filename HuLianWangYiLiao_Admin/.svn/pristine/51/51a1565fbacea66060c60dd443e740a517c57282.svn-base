		...
		<table id="sample-table-1" class="table  table-bordered dataTable"> 
			<thead>
				<tr>
					...
					<th field="doctor_id"   onclick="querySort(this,'queryInquiryLog')"  >
						人员
					</th>
					<th field="log_source"   onclick="querySort(this,'queryInquiryLog')"  >
						记录来源
					</th>
					<th field="log_history_present_illness"   onclick="querySort(this,'queryInquiryLog')"  >
						病史
					</th>
					<th field="lof_diagnosis" width="120"  onclick="querySort(this,'queryInquiryLog')"  >
						诊断
					</th>
					<th field="unprescribed_reasons" width="120"  onclick="querySort(this,'queryInquiryLog')"  >
						未开原因
					</th>
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr id="tr${info.log_id}">
						...
						<td style="vertical-align: middle;">
							...
							onclick="openlookInfo('${info.log_id}','${info.log_name}')">查看</button>
						</td>
					</tr>
				</c:forEach>
			</tbody>
...
<script src="${root}/form/inquiryLogs/js/inquirylog_main.js?<%=Math.random()%>"></script>
