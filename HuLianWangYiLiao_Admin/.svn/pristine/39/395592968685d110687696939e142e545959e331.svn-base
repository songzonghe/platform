				...
				<div class="widget-box">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="reportForm_checklist_index.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								<input style="display: none;" id="mapStr" name="mapStr">
		 						<fieldset>
									...
									<option value="">请选择</option>
										<option <c:if test="${map.type=='1'}">selected="selected"</c:if> value="1">已开检验检查</option>
										<option <c:if test="${map.type=='2'}">selected="selected"</c:if> value="2">撤销检验检查</option>
									</select>
									
									<button type="submit" onclick="queryChecklist(0)" class="btn btn-sm btn-success">
										搜索
			...
			<tbody id="tbodyData">
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr>
						...
						<td style="vertical-align: middle;">
							<c:if test="${info.type=='1'}">已开检验检查</c:if>
							<c:if test="${info.type=='2'}">撤销检验检查</c:if>
						</td>
						<td class="printing"  style="vertical-align: middle;color: #f9a821;">
							<span id="${info.checklist_id}" style="cursor: pointer;">查看打印</span>
						</td>
					</tr>
				</c:forEach>
			</tbody>
...
<div class="backgrounde8f3fb backgroundfff" style="position: fixed;background: #fff; z-index: 3341;  top: 30px; left: 50%;  transform: translateX(-50%);width: 600px; border-radius: 0; font-size: 15px; max-height: 86%;overflow: hidden; overflow-y: auto;display:none">
	...
	<div class="ChecklistDetails"></div>
</div>
...
jQuery('.printing').on('click','span',function(){
	...
	printing('2',id);//// 0 历史处方详情  1 历史处方打印 2检验打印  3 建议指导打印 4 住院建议打印
