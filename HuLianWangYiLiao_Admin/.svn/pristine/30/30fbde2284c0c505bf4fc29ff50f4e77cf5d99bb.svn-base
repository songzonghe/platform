...
<div class="widget-main no-padding" style="width: 880px">
	<form class="form-horizontal" isValidate="y" onsubmit="return false;">
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						<input style="margin-top: 10px;" type="radio" <c:if test="${map.log_type=='1'}">checked="checked"</c:if> name="update_log_type" value="1" >视频问诊&nbsp;&nbsp;
					<input type="radio" name="update_log_type" value="2" <c:if test="${map.log_type=='2'}">checked="checked"</c:if> onclick="$('#isUpdate').attr('value','y')">图片问诊
					</span>
				</div>
			</div>
		</div>
				...
				<span class="block input-icon input-icon-right">
					<select  id="update_store_id" class="col-xs-6" data-validate="">
						...
						<c:forEach items="${shopList}" var="info">
							<option value="${info.store_id}"<c:if test="${info.drug_id==map.drug_id2}">selected</c:if>>${info.store_name}(${info.pro_batch_number})</option>
						</c:forEach>
					</select>
				</span>
			</div>
				...
				<span class="block input-icon input-icon-right">
					<select  id="update_user_id" class="col-xs-6" data-validate="">
						...
						<c:forEach items="${personList}" var="info">
							<option value="${info.user_id}" <c:if test="${info.user_id==map.user_id}">selected</c:if>>${info.user_name}</option>
						</c:forEach>
				...
				<span class="block input-icon input-icon-right">
					<select  id="update_inquiry_patient_id" class="col-xs-12" data-validate="">
						...
						<c:forEach items="${patientList}" var="info">
							<option value="${info.inquiry_patient_id}"<c:if test="${info.inquiry_patient_id==map.inquiry_patient_id}">selected</c:if>>${info.inquiry_patient_name}</option>
						</c:forEach>
					</select>
				</span>
			</div>
		...
		<div class="form-group col-sm-6 field doctor">
			<label class="col-sm-4 control-label no-padding-right" onchange="doctorList()">
				医生
			</label>
			<div class="col-sm-6 field">
				<span class="block input-icon input-icon-right">
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						<input type="text" placeholder="记录" id="update_log_source" class="col-xs-12" data-validate="required:记录不能为空哦！" value="${map.log_source}" >
					</span>
				</div>
			</div>
		<div class="form-group col-sm-6 field">
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						<textarea type="text" placeholder="现病史" id="update_log_present_illness" class="col-xs-12" data-validate="required:现病史不能为空哦！" value="${map.log_present_illness}" >${map.log_present_illness}</textarea>
					</span>
				</div>
			</div>
		<div class="form-group col-sm-6 field">
			...
			<div class="col-sm-8 field">
				<span class="block input-icon input-icon-right"> 
					<textarea type="text" placeholder="既往史" id="update_log_history_present_illness" class="col-xs-12" data-validate="required:既往史不能为空哦！" value="${map.log_history_present_illness}" >${map.log_history_present_illness}</textarea>
				</span>
			</div>
		</div>
		</div>
		...
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						<textarea type="text" placeholder="诊断" id="update_lof_diagnosis" class="col-xs-12" data-validate="required:诊断不能为空哦！" value="${map.lof_diagnosis}" >${map.lof_diagnosis}</textarea>
					</span>
				</div>
			</div>
		<div class="form-group col-sm-6 field">
				...
				<div class="col-sm-8 field">
					<span class="block input-icon input-icon-right"> 
						<textarea class="col-xs-12" id="update_lof_proposal" placeholder="检验建议" data-validate="required:检验建议不能为空哦！" >${map.lof_proposal}</textarea>
					</span>
				</div>
			</div>
		</div>
