				...
				<div class="widget-header">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="utilSpecification_index.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								...
								<fieldset>
									...
									<input type="text" id="query_specification_name" placeholder="规范名称" value="${map.specification_name}">
									<button type="submit" onclick="queryUtilSpecification(0)" class="btn btn-sm btn-success">
										搜索
		...
		<table id="sample-table-1" class="table  table-bordered dataTable"> 
			<thead>
				<tr>
					...
					<th field="specification_name"   onclick="querySort(this,'queryUtilSpecification')"  >
						规范名称
					</th>
					<th field="specification_details"   onclick="querySort(this,'queryUtilSpecification')"  >
						规范说明
					</th>
					<th field="specification_sort"   onclick="querySort(this,'queryUtilSpecification')"  >
						规范排序
					</th>
					<th field="gxsj" width="170" onclick="querySort(this,'queryUtilSpecification')">
						修改时间
					</th>
					<th field="bz"  onclick="querySort(this,'queryUtilSpecification')">
						备注
					</th>
					<th width="180">
						操作
			...
			<tbody id="tbodyData">
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr id="tr${info.specification_id}">
						<td index="tdIndex" style="vertical-align: middle;">
							${i.index+1}
						</td>
						<td style="vertical-align: middle;">
							${info.specification_name}
						</td>
						<td style="vertical-align: middle;">
							${info.specification_details}
						</td>
						<td style="vertical-align: middle;">
							${info.specification_sort}
						</td>
						<td style="vertical-align: middle;">
							${info.gxsj}
						</td>
						...
						<td style="vertical-align: middle;">
							<button class="btn btn-xs btn-info" onclick="openEditPage('${info.specification_id}')" title="编辑">
								编辑
							</button>
							<button class="btn btn-xs btn-danger" onclick="confirmDeleteDate('${info.specification_id}','${info.specification_id}')" title="删除">
								删除
							</button>
						</td>
					</tr>
...
<c:set var="cx" value="queryUtilSpecification" />
...
<script src="${root}/form/util/specification/js/utilspecification_main.js?<%=Math.random()%>"></script>
</body>
</html>