				...
				<div class="widget-header">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="utilMedicalAdvice_index.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								...
								<fieldset>
									...
									<input type="text" id="query_advice_name" placeholder="医嘱名称" value="${map.advice_name}">
									<button type="submit" onclick="queryUtilMedicalAdvice(0)" class="btn btn-sm btn-success">
										搜索
									...
									<button type="button" onclick="openImportBoxPage('util_medical_advice','queryUtilMedicalAdvice')" class="btn btn-sm btn-info">
										导入
										<i class="ace-icon fa fa-plus icon-on-right bigger-110"></i>
									</button>
								</fieldset>
		...
		<table id="sample-table-1" class="table  table-bordered dataTable"> 
			<thead>
				<tr>
					...
					<th field="advice_name"   onclick="querySort(this,'queryUtilMedicalAdvice')"  >
						医嘱名称
					</th>
					<th field="advice_sort"   onclick="querySort(this,'queryUtilMedicalAdvice')"  >
						医嘱排序
					</th>
					<th field="gxsj" width="170" onclick="querySort(this,'queryUtilMedicalAdvice')">
						修改时间
					</th>
					<th field="bz"  onclick="querySort(this,'queryUtilMedicalAdvice')">
						备注
					</th>
					<th width="180">
						操作
			...
			<tbody id="tbodyData">
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr id="tr${info.advice_id}">
						...
						<td style="vertical-align: middle;" type="num" >
							${info.advice_sort}
						</td>
						<td style="vertical-align: middle;">
							${info.gxsj}
						...
						<td style="vertical-align: middle;">
							<button class="btn btn-xs btn-info" onclick="openEditPage('${info.advice_id}')" title="编辑">
								编辑
							</button>
							<button class="btn btn-xs btn-danger" onclick="confirmDeleteDate('${info.advice_id}','${info.advice_id}')" title="删除">
								删除
							</button>
						</td>
					</tr>
...
<c:set var="cx" value="queryUtilMedicalAdvice" />
...
<script src="${root}/form/util/medical/advice/js/utilmedicaladvice_main.js?<%=Math.random()%>"></script>
</body>
</html>