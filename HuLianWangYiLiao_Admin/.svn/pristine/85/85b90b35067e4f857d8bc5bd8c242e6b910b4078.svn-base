				...
				<div class="widget-header">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="utilMeasuringCondition_index.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								...
								<fieldset>
									...
									<input type="text" id="query_condition_name" placeholder="条件名称" value="${map.condition_name}">
									<button type="submit" onclick="queryUtilMeasuringCondition(0)" class="btn btn-sm btn-success">
										搜索
		...
		<table id="sample-table-1" class="table  table-bordered dataTable"> 
			<thead>
				<tr>
					...
					<th field="condition_name"   onclick="querySort(this,'queryUtilMeasuringCondition')"  >
						条件名称
					</th>
					<th field="condition_sort"  type="num"  onclick="querySort(this,'queryUtilMeasuringCondition')"  >
						条件排序
					</th>
					<th field="gxsj" width="170" onclick="querySort(this,'queryUtilMeasuringCondition')">
						修改时间
					</th>
					<th field="bz"  onclick="querySort(this,'queryUtilMeasuringCondition')">
						备注
					</th>
					<th width="180">
						操作
			...
			<tbody id="tbodyData">
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr id="tr${info.condition_id}">
						...
						<td style="vertical-align: middle;">
							<button class="btn btn-xs btn-info" onclick="openEditPage('${info.condition_id}')" title="编辑">
								编辑
							</button>
							<button class="btn btn-xs btn-danger" onclick="confirmDeleteDate('${info.condition_id}','${info.condition_id}')" title="删除">
								删除
							</button>
						</td>
					</tr>
...
<c:set var="cx" value="queryUtilMeasuringCondition" />
...
<script src="${root}/form/util/measuring/condition/js/utilmeasuringcondition_main.js?<%=Math.random()%>"></script>
</body>
</html>