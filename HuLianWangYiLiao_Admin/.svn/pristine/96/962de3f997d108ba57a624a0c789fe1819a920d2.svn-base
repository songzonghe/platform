				...
				<div class="widget-header">
					...
					<div class="widget-body">
						<div class="widget-main no-padding">
							<form action="utilOccupation_indexs.do" id="queryForm" method="post">
								<!-- 参数隐藏 用于页面传值-->
								...
								<fieldset>
									...
									<input type="text" id="query_occupation_name" placeholder="职业名称" value="${map.occupation_name}">
									<button type="submit" onclick="queryUtilOccupation(0)" class="btn btn-sm btn-success">
										搜索
		...
		<table id="sample-table-1" class="table  table-bordered dataTable"> 
			<thead>
				<tr>
					...
					<th field="occupation_name"   onclick="querySort(this,'queryUtilOccupation')"  >
						职业名称
					</th>
					<th field="occupation_sort"  type="num"  onclick="querySort(this,'queryUtilOccupation')"  >
						职业排序
					</th>
					<th field="gxsj" width="170" onclick="querySort(this,'queryUtilOccupation')">
						修改时间
					</th>
					<th field="bz"  onclick="querySort(this,'queryUtilOccupation')">
						备注
					</th>
					<th width="180">
						操作
			...
			<tbody id="tbodyData">
				...
				<c:forEach items="${listMap}" varStatus="i" var="info">
					<tr id="tr${info.occupation_id}">
						...
						<td style="vertical-align: middle;">
							<button class="btn btn-xs btn-info" onclick="openEditPage('${info.occupation_id}')" title="编辑">
								编辑
							</button>
							<button class="btn btn-xs btn-danger" onclick="confirmDeleteDate('${info.occupation_id}','${info.occupation_id}')" title="删除">
								删除
							</button>
						</td>
					</tr>
...
<c:set var="cx" value="queryUtilOccupation" />
...
<script src="${root}/form/util/occupation/js/utiloccupation_main.js?<%=Math.random()%>"></script>
</body>
</html>