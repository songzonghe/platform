 ...
 * 操作人： 苏洪伟
 */ 
@Service("utilIcdServiceImp")
public class UtilIcdServiceImp implements UtilIcdService {
	@Resource
	...
	private void getSQL(Map<String, String> mapStr,StringBuffer sql,Map<String, Object> mapSQLParameter) throws Exception {
		sql.append("select t.icd_id,t.hospital_id,t.icd_name,t.icd_sort,t.gxsj,t.bz,t.lrsj,t.zt,t.lrzh");
		sql.append(" from util_icd t where t.hospital_id=:hospital_id");
		mapSQLParameter.put("hospital_id", mapStr.get("login_hospital_id"));
		if (UtilValiDate.isEmpty(mapStr.get("icd_id"))) {
			sql.append(" and t.icd_id=:icd_id ");
			mapSQLParameter.put("icd_id", mapStr.get("icd_id"));
		}else {
			if (UtilValiDate.isEmpty(mapStr.get("icd_name"))) {
				sql.append(" and t.icd_name like :icd_name");
	...
	public Map<String, Object> add_Data(Map<String, String> mapStr) throws Exception{
		StringBuffer sql = new StringBuffer(); Map<String, Object> mapSQLParameter = new HashMap<String, Object>();
		sql.append("insert into util_icd");
		sql.append("( hospital_id,icd_name,icd_sort");
		sql.append(UtilSql.getFieldInsertKey());
		sql.append("values( :hospital_id,:icd_name,:icd_sort");
		sql.append(UtilSql.getFieldInsertVal());
		mapSQLParameter.put("hospital_id", mapStr.get("login_hospital_id"));
		mapSQLParameter.put("icd_name", mapStr.get("icd_name"));
		mapSQLParameter.put("icd_sort", mapStr.get("icd_sort"));
		UtilSql.setMapVal(mapSQLParameter, mapStr);
		mapStr.put("icd_id", this.gyDAOimp.exeSqlGetId(sql, mapSQLParameter));
	...
	public Map<String, Object> update_Data(Map<String, String> mapStr) throws Exception{
		...
		StringBuffer sql = new StringBuffer(); Map<String, Object> mapSQLParameter = new HashMap<String, Object>();
		sql.append("update util_icd set gxsj=:gxsj");
		if(null!=mapStr.get("hospital_id")){
			sql.append(",hospital_id=:hospital_id");
			mapSQLParameter.put("hospital_id", mapStr.get("login_hospital_id"));
		}
		if(null!=mapStr.get("icd_name")){
			sql.append(",icd_name=:icd_name");
			mapSQLParameter.put("icd_name", mapStr.get("icd_name"));
		}
		if(null!=mapStr.get("icd_sort")){
			sql.append(",icd_sort=:icd_sort");
			mapSQLParameter.put("icd_sort", mapStr.get("icd_sort"));
		}
		...
		sql.append(" where icd_id=:icd_id");
		mapSQLParameter.put("icd_id", mapStr.get("icd_id"));
	...
	public void delete_Data(Map<String, String> mapStr) throws Exception{
		StringBuffer sql = new StringBuffer(); Map<String, Object> mapSQLParameter = new HashMap<String, Object>();
		sql.append("delete from util_icd where icd_id=:icd_id");
		mapSQLParameter.put("icd_id", mapStr.get("id"));
		this.gyDAOimp.exeSqlBool(sql, mapSQLParameter);
